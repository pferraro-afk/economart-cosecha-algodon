...
def style_rinde(df, col="rinde_desmote"):
    return df.style.applymap(color_rinde, subset=[col])
...
with st.sidebar:
    ...
    sel_camp = st.selectbox("Campaña", campanias)
...
c1.metric("Algodón Bruto",   f"{df['pesoneto'].sum()/1000:,.1f} Tn")
...
with col_g1:
    ...
    fig = px.bar(
        ...
        orientation="h", labels={"bruto_tn":"Tn","establecimiento":""},
...
with col_g2:
    ...
    fig2 = px.bar(
        ...
        orientation="h", labels={"rinde_desmote":"%","establecimiento":""},
        color_discrete_sequence=px.colors.qualitative.Set2,
    )
...
disp_campo = vc.rename(columns={
    ...
    "fardos":"Fardos","ppf_kg":"Peso Prom Fardo (kg)","remitos":"Remitos",
})[["Empresa","Campo","Sup (ha)","Bruto (Tn)","Rinde Bruto (kg/ha)","Fibra (Tn)","Rinde Desmote (%)","Fardos","Peso Prom Fardo (kg)"]]
st.dataframe(
...
disp_lote = vl.rename(columns={
    ...
    "fibra_tn":"Fibra (Tn)","rinde_desmote":"Rinde Desmote (%)",
    "fardos":"Fardos",
})[["Empresa","Campo","Lote","Sup (ha)","Bruto (Tn)","Rinde Bruto (kg/ha)","Fibra (Tn)","Rinde Desmote (%)","Fardos"]]
st.dataframe(
...
})[["Desmotadora","Empresa","Campo","Entregado (kg)","Fibra (kg)","Rinde (%)","Fardos","Peso Prom Fardo (kg)"]]
st.dataframe(
